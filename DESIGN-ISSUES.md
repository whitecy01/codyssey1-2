...
## 오류 2. "괜찮다"고 통과시킨 설정의 절반이 반드시 죽는다

### `CPU_MAX_OCCUPY` 는 상한이 아니라 목표치다

이름은 "최대 점유율"처럼 읽히지만, 실제로는 **워커가 부하를 끌어올릴 목표**다.
설정한 숫자까지 올렸다가 다시 내리는 것을 반복한다.

`CPU_MAX_OCCUPY=30` 으로 실행하면 30 에서 꺾여 내려온다.
([`evidence/cpu-after/agent_app.log`](evidence/cpu-after/agent_app.log))

```text
부하
 50 ┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈  ← 이 선을 넘으면 Watchdog 이 죽인다
 30      ╱╲        ╱╲
  5   ╱      ╲__╱      ╲   ← 50 에 닿지 않는다 → 생존
```
...
`CPU_MAX_OCCUPY=100` 으로 실행하면 100 을 향해 올라가다,
그 길목에 있는 50 을 넘는 순간 종료된다.
([`evidence/cpu-before/agent_app.log`](evidence/cpu-before/agent_app.log))

```text
부하
100                      ↗ (목표)
 50 ┈┈┈┈┈┈┈┈┈┈╳┈┈┈┈┈┈┈┈  ← 여기서 죽는다. 100 에는 영원히 못 간다
  5   ╱╱╱╱╱╱
```
...
Watchdog 이 죽이는 기준은 `CPU_MAX_OCCUPY` 가 아니라 **50% 고정**이다.
이 숫자는 앱이 기동할 때 스스로 알려준다.

```text
 [ CPU    ] Limit: 100%  		[ WARNING: Recommend Under 50% ]
                                                    ^^^^^^^^
                                     "50% 미만으로 하라" — 앱이 제시한 안전선
```

즉 앱은 **"50% 미만으로 쓰라"고 권하면서, 동시에 100 까지 설정할 수 있게 열어둔다.**
권장선과 입력 가능 범위가 따로 논다.

```
CPU_MAX_OCCUPY  = 사용자가 정함 (부트 검사가 10~100 을 통과시킴)
Watchdog 임계   = 코드에 박혀 있음 (50 고정, 배너의 "Under 50%" 와 같은 값)
```

두 숫자가 서로를 참조하지 않기 때문에, 권장선을 무시한 값을 넣어도 검사가 막지 않는다.

그래서 이런 일이 생긴다.

| 설정값 | 부하가 향하는 곳 | 50 선을 넘나 | 결과 |
| --- | --- | --- | --- |
| 10 ~ 50 | 50 이하 | 안 넘음 | 생존 |
| **51 ~ 100** | 50 보다 위 | **반드시 넘음** | **반드시 종료** |

즉 **부트 검사가 "괜찮다"고 통과시킨 값의 절반(51~100)이,
실행하면 예외 없이 죽는 값**이다.

```text
[6/6] Verifying Mission Environment       [OK]
All Boot Checks Passed!            ← CPU_MAX_OCCUPY=70 이어도 여기까지 통과한다
```

우발적인 부하 급증이 아니다. 목표치를 안전 임계보다 높게 잡는 것을
검증 단계에서 그대로 허용했기 때문에 **종료가 예정된 구성**이다.

### 경계값 50 은 통과하지만, 여유가 없다

50 은 애매하다. 부하가 50 까지 올라가는데 죽는 선도 50 이기 때문이다.
직접 실행해 확인했다.
...
[CpuWorker] Current Load: 40.26%      ← 위반 없이 내려온다
```

죽는 조건은 `부하 > 50` 이라 50 자체는 안전하다.
(실제로 위반이 관측된 값은 50.05% 와 52.48% 로 모두 50 초과였다.)

**다만 여유가 0 이다.** 50.01 만 되어도 죽는다.
지금은 워커가 목표치를 정확히 지켜서 살아남을 뿐이다.

메모리 쪽에서는 실제로 오버슈트가 일어났다 — `MEMORY_LIMIT=256` 인데 힙이 275MB 까지
올라간 뒤에야 감지되었다(25MB 단위로 할당하기 때문). CPU 에서도 같은 일이 생기지 않는다는
보장은 없다. **"권장할 수 있는 최댓값"이 곧 "죽기 직전"인 값을 권장선으로 삼는 것 자체가
위험하다.**

### 곁가지 — 두 배너가 서로 다른 규칙을 쓴다

위 경계 실험에서 드러난 부수적 문제다.

| 배너 문구 | 경계값을 넣으면 | 경계 포함 여부 |
| --- | --- | --- |
| `Recommend Over 256MB` | `MEMORY_LIMIT=256` → **`WARNING`** | 256 **제외** |
| `Recommend Under 50%` | `CPU_MAX_OCCUPY=50` → **`OK`** | 50 **포함** |

같은 형식인데 `Over` 는 경계를 빼고 `Under` 는 넣는다.
게다가 "Under 50%"(50 미만)라고 써놓고 50 을 통과시키므로, **문구와 실제 동작이 어긋난다.**

실제 동작(50 초과에서 죽음)이 맞고 문구가 부정확한 것이므로, 고쳐야 할 쪽은 문구다.
`Recommend >= 257MB`, `Recommend <= 50%` 처럼 부등호로 적으면 사라질 모호함이다.
현재는 문구만 읽어서는 알 수 없어, 두 변수 모두 경계값을 직접 실행해 본 뒤에야 확정할 수 있었다.

### 운영상의 영향

**조치 방향이 메모리와 정반대**라는 점이 혼란을 만든다.

| 변수 | 장애를 막으려면 | 성격 |
| --- | --- | --- |
| `MEMORY_LIMIT` | **올린다** (50 → 512) | 넘으면 안 되는 **상한** |
| `CPU_MAX_OCCUPY` | **내린다** (100 → 30) | 도달하려 애쓰는 **목표치** |

이름이 둘 다 "한도(LIMIT / MAX)"처럼 읽히지만 성격이 반대다.
"한도니까 넉넉히 잡자"는 판단이 메모리에서는 맞고 CPU 에서는 곧바로 종료로 이어진다.
...
6. 권장 범위를 `Over` / `Under` 대신 **부등호로 표기**해 경계 포함 여부를 명확히 한다.
