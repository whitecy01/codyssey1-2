...
| 2 | CPU Latency | [**02-cpu.md**](reports/02-cpu.md) | `CPU_MAX_OCCUPY` | 100 → 30 · 생존 **28초 → 120초+** |
...
2. **CPU 장애는 `top`/`ps`로 관측되지 않는다.**
   앱이 `Current Load: 50.05%` 를 보고하고 Watchdog이 종료를 결정하는 그 순간,
   OS가 측정한 이 프로세스의 누적 CPU 시간은 **22초 동안 0.22초(≈1%)** 였다.
...
│   ├── monitor.sh          프로세스 단위 관제 (CPU/RSS/스레드/상태를 2초 간격 기록)
...
| `oom-mid` | **256** | 30 | false | 34초 후 SIGKILL (경계값 검증) |
| `oom-after` | **512** | 30 | false | 300초 생존, 회수 4회 |
| `cpu-before` | 512 | **100** | false | 28초 후 SIGTERM |
